...
def main():
    ...
    L = 6
    B = 2
    weight_map = torch.rand((B, L, H, W), dtype=torch.float32).to(device)
    kernel_map = torch.rand((B, L, H, W), dtype=torch.float32).to(device)
    imgs_in = torch.rand((B, H, W, 4), dtype=torch.float32).to(device)
    # for h in range(H):
    #     imgs_in[h] = h + 1
    imgs_out = torch.zeros((B, H, W, 4), dtype=torch.float32).to(device)
    requires_grad = True
    ...
    def f(inputs):
        ...
        return _denoiser.filtering_autograd(weight_map, kernel_map, imgs_in, requires_grad=requires_grad)
